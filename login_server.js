...
app.get('/transaction', (req, res) => {
    ...
    const query = 'SELECT reg_firstname, reg_middlename, reg_lastname FROM user_registration WHERE registration_id = ?';
    // Query to retrieve reservation data for the specific equipment (facility ID = 1001)
    const reservationQuery = 'SELECT * FROM reservation WHERE fk_reservation_registration_id = ? AND fk_equipment_facility_id = ?';
    
    // Query for user information
    connection.query(query, [user.registration_id], (err, userResults) => {
        if (err) {
        ...
        // Now query for the user's reservations for equipment facility ID = 1001
        connection.query(reservationQuery, [user.registration_id, 1001], (err, reservationResults) => { // Pass the facility ID here
            if (err) {
            ...
            res.render('transaction', {
                ...
                reservations: reservationResults    // Reservations data with facility info
            });
        });
    });
});
...
app.post('/admin_approve_request', (req, res) => {
    ...
    // Update query to change the reservation status
    const query = `UPDATE reservation SET reservation_status = ? WHERE reservation_id = ? AND reservation_status = 'pending'`;
    ...
    connection.query(query, [status, reservation_id], (err, result) => {
        ...
        if (result.affectedRows === 0) {
            return res.status(404).json({ success: false, message: 'Reservation not found or already updated' });
        }
...
app.get('/admin_approve_request', (req, res) => {
    ...
    const reservationQuery = `
        ...
        FROM reservation 
        JOIN equipment_table ON reservation.fk_equipment_facility_id = equipment_table.equipment_facility_id
        WHERE reservation.reservation_status = 'PENDING'`;

    connection.query(reservationQuery, (err, reservationResults) => {
        if (err) {
            console.error('Error fetching reservations:', err);
            return res.status(500).send('Error retrieving reservations from MySQL');
        }

        // Render the admin_approve_request page with the reservation data
